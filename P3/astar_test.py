...
import unittest
from astar_search import MapSpace


class AstarUnitTests(unittest.TestCase):
    def test_cases(self):
        m40 = self._get_map_data()
        astar = MapSpace(nodes=m40['intersections'], edges=m40['roads'])
        self.assertEqual([5], astar.get_astar_path(src=5, dst=5))
        self.assertEqual([5, 16, 37, 12, 34], astar.get_astar_path(src=5, dst=34))
        self.assertEqual([8, 14, 16, 37, 12, 17, 10, 24], astar.get_astar_path(src=8, dst=24))
